...
반면에, 명확한 business rule, 단순 lookup, sorting, deterministic calculation처럼 입력과 출력 관계를 규칙으로 정확하게 정의할 수 있는 문제는 일반적인 software algorithm이나 rule-based system이 더 단순하고 검증하기 쉬울 수 있습니다. (모든 문제에 AI가 정답이 아니라고! 반도체 값만 올리는 주범)
